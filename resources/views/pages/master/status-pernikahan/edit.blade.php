...
@section('title', 'Edit Data Status Pernikahan')
@section('description', 'Edit Data Status Pernikahan - Rekam Medis')
@section('content')
    <div class="p-4 sm:ml-64">
        <div class="rounded-lg border-2 border-dashed border-gray-200 p-4">
            <h1 class="mb-4 rounded bg-white py-5 pl-3 text-3xl font-bold text-gray-700 shadow">Edit Data Status Pernikahan
            </h1>
            <div class="rounded-lg bg-white p-6 shadow">
                <form action="{{ route('master.status-pernikahan.update', $statusPernikahan->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="mb-6">
                        <label for="nama" class="mb-2 block text-sm font-medium text-gray-900">Nama Status Pernikahan
                        ...
                        <input type="text" id="nama" name="nama"
                            ...
                            placeholder="Masukkan nama status pernikahan" value="{{ old('nama', $statusPernikahan->nama) }}"
                            required>
                        @error('nama')
