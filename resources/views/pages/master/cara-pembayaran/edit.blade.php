...
@section('title', 'Edit Data Cara Pembayaran')
@section('description', 'Edit Data Cara Pembayaran - Rekam Medis')
@section('content')
    <div class="p-4 sm:ml-64">
        <div class="rounded-lg border-2 border-dashed border-gray-200 p-4">
            <h1 class="mb-4 rounded bg-white py-5 pl-3 text-3xl font-bold text-gray-700 shadow">Edit Data Cara Pembayaran
            </h1>
            <div class="rounded-lg bg-white p-6 shadow">
                <form action="{{ route('master.cara-pembayaran.update', $data->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="mb-6">
                        <label for="nama" class="mb-2 block text-sm font-medium text-gray-900">Nama Cara Pembayaran <span
                        ...
                        <input type="text" id="nama" name="nama"
                            ...
                            placeholder="Masukkan nama cara pembayaran" value="{{ old('nama', $data->nama) }}" required>
                        @error('nama')
