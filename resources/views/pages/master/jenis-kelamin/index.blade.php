...
@section('title', 'Master Data Jenis Kelamin')
@section('description', 'Master Data Jenis Kelamin - Rekam Medis')
@section('content')
    <div class="p-4 sm:ml-64">
        <div class="rounded-lg border-2 border-dashed border-gray-200 p-4">
            <h1 class="mb-4 rounded bg-white py-5 pl-3 text-3xl font-bold text-gray-700 shadow">Master Data Jenis Kelamin
            </h1>
            <div class="mt-10 flex flex-wrap items-center justify-between pb-4">
                <div class="flex gap-4">
                    <a type="button" href="{{ route('master.jenis-kelamin.create') }}"
                ...
                <table id="search-table"
                    ...
                    <thead class="border-b-1 bg-white text-xs uppercase text-gray-700">
                        <tr>
                            ...
                            <th class="px-6 py-3">Nama Jenis Kelamin</th>
                    ...
                    <tbody>
                        @if ($data->isEmpty())
                            <tr class="bg-white hover:bg-gray-100">
                                <td colspan="4" class="p-4 text-center">Tidak ada data jenis kelamin</td>
                            </tr>
                        @else
                            @foreach ($data as $index => $item)
                                <tr class="bg-white hover:bg-gray-100">
                                    ...
                                    <td class="flex gap-3 px-6 py-4">
                                        <div>
                                            <a href="{{ route('master.jenis-kelamin.edit', $item->id) }}"
                                                class="cursor-pointer rounded bg-blue-500 px-2 py-1 text-xs font-bold text-white hover:bg-blue-700">
                                                Edit
                                            </a>
                                        </div>
                                        <div>
                                            <form action="{{ route('master.jenis-kelamin.destroy', $item->id) }}"
